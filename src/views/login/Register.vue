<template>
  <div class="loginRegister">
    <Header
      :msg-header="$t('lang.titles.register')"
    ...
    >
      ...
      <div class="linkGroup">
        ...
        <Button theme="primary" :width="590" @click="registSubmit">{{ $t('lang.register.register') }}</Button>
        <router-link to="login" class="pink">{{ $t('lang.register.login') }}</router-link>
      </div>
    </Container>
    <Modal v-model="bankIdEmpty" :title="$t('lang.register.bankIdEmpty')" type="primary">
      <p>{{ this.$t('lang.register.alert12') }}</p>
      <div slot="footer">
        <Button theme="primary" @click="bankIdEmpty = false">{{ this.$t('lang.home.button.OK') }}</Button>
      </div>
    </Modal>
  </div>
</template>
